...
export default async function StatusPage({ params }: StatusPageProps) {
  ...
  const allOperational = services.every((s: Service) => s.status === "OPERATIONAL")
      ...
      <div className="container max-w-5xl mx-auto px-4">
        ...
        <Card className="mb-8">
          <CardHeader className="pb-2">
            <CardTitle className={`text-2xl flex items-center ${allOperational ? "text-green-500" : "text-red-500"}`}>
              <div className={`w-3 h-3 rounded-full mr-2 ${allOperational ? "bg-green-500" : "bg-red-500"}`} />
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {services.map((service: Service) => (
                <div key={service.id} className="flex items-center justify-between border-b py-2">
        ...
        {activeIncidents.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-4 flex items-center">
              <AlertCircle className="mr-2 h-5 w-5 text-red-500" /> Active Incidents
            </h2>
                    ...
                    {incident.updates.length > 0 && (
                      <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-md">
                        <div className="flex items-center text-sm mb-1">
                          <Clock className="h-3 w-3 mr-1" />
                          {format(new Date(incident.updates[0].createdAt), "h:mm a")}
                          <span className="ml-2">
                            <IncidentStatusBadge status={incident.updates[0].status} />
                          </span>
                        </div>
        ...
        {upcomingMaintenance.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-bold mb-4 flex items-center">
              <Calendar className="mr-2 h-5 w-5 text-blue-500" /> Scheduled Maintenance
            </h2>
            <div className="space-y-4">
                  ...
                  <Card key={maintenance.id}>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            ...
                            <span className="text-sm text-muted-foreground">{maintenance.service.name}</span>
                          </div>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="flex items-center mb-2">
                        ...
                        {format(new Date(maintenance.startTime), "MMM d, h:mm a")} - {format(new Date(maintenance.endTime), "MMM d, h:mm a")}
                      </div>
